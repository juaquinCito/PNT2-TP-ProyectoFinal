  ...
  methods: {
    ...
    async cargarTurno() {
      ...
      try {
        ...
        this.date = date;
        this.especialista = turno.especialista.name;
      } catch (error) {
        alert(error);
      }
    },
    async onSubmit() {
      ...
      // Transformar la fecha a UNIX timestamp(La pasa a string para poder mostrarla, tiene un bugcito)
      const date = Math.floor(new Date(this.date).getTime() / 1000);
      const elem = {
        date: date,
        especialista: { name: this.especialista },
...
<template>
  ...
  <div class="container">
    ...
    <form v-on:submit.prevent="onSubmit">
      ...
      <VueDatePicker :min-date="new Date()" model-type="MM/dd/yyyy HH:mm" class="pickerDate" v-model="date">
      </VueDatePicker>
